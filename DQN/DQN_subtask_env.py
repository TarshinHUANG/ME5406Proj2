...
GATE_POS = (
700, 700)  # the initial location of gate (valid if random location is closed), this can be n-D array for n gates
...
class Ball_env(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Box(low=-10000, high=10000, shape=(6,), dtype=np.float32)
    ...
    #  @staticmethod
    def action_two_wheel(self, action_class):
        action = [0, 0]
    ...
    def random_action(self):
        action = [random.uniform(-2, 2), random.uniform(-2, 2)]  # change
        return action
