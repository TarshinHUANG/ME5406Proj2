...
DT = 0.1  # timespan for each round
...
class Viewer(pyglet.window.Window):
    ...
    def on_key_press(self, symbol, modifiers):
        if symbol == key.UP:
            move = [0.04, 0.04]  # Clockwise
        elif symbol == key.DOWN:
            move = [-0.04, -0.04] # anti-above-direction
        elif symbol == key.LEFT:
            move = [-0.02, 0.02] # anti-clockwise
        elif symbol == key.RIGHT:
            move = [0.02, -0.02] # anti-above-direction
        else:
            move = [1, -1]
